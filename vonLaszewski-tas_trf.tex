...
To answer these questions, first we need a process to quantify the scientific outcome for the individual research units. Secondly we need to define and generate metrics to measure the scientific impact for individual units and higher level aggregated entities. Finally we could correlate the impact metrics to the consumed resources, to get an insight on how the computing facility benefits and impacts the science conducted utilizing its resources. In this paper, we present a framework that addresses these questions and processes. We have to point out that measuring scientific impact can be quite controversial and that the presented results do not necessarily represent an absolute measure of the impact of a scientific project, but rather the results we present represent one of many factors that together define the scientific impact.
...
\item Based on the consolidated relevant bibliometrics data various scientific impact metrics are derived for users and other aggregated levels such as projects, field of science.
\item Statistical analyses have been done correlating the impact metrics with projects/proposals, field of science, and allocation data to show how XSEDE impact the sciences conducted utilizing XSEDE resources.
\item The results are presented via our lightweight portal, and are also exposed via database integration or RESTful services to our collaborator teams such as XDMoD, XSEDE portal. For example, we are exposing the publication data via RESTful service API to XSEDE portal team as a publication suggestion service. This could help ease XSEDE users identifying and curating their XSEDE related publications.
\end{enumerate} 
